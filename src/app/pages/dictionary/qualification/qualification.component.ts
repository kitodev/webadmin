...
export class QualificationComponent implements OnInit {
  ...
  listQualificationToEdit: Qualification;
  ...
  ngOnInit(): void {
    ...
    if(!this.isNew) {
      this.loopbackService
        .findQualificationById(this.qualificationId)
        .pipe(takeUntil(this.unsubscribe))
        .subscribe((response) => {
  ...
  buildForm() {
    const qualificationFormModel: FormModel<Qualification> = {
      id: [this.listQualificationToEdit ? this.listQualificationToEdit.id : 1, Validators.required],
  ...
  reset(): void {
    Swal.fire({
      ...
      if (result.value) {
        ...
        this.router.navigate(['/dictionary/qualifications/list'])
            .then();
        this.buildForm();
      }
    })
