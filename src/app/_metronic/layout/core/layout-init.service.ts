...
export class LayoutInitService {
  ...
  private initHeader() {
    let headerWidth = this.layout.getProp('header.width');
    headerWidth = 'fixed';
  ...
  private initToolbar() {
    ...
    let widthClass = this.layout.getProp('toolbar.width') as string;
    widthClass = 'fixed';
    this.layout.setCSSClass(
      'toolbarContainer',
  ...
  private initAside() {
    let display = this.layout.getProp('aside.display') as boolean;
    display = false;
    if (!display) {
      return;
    }
